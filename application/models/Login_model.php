...
class Login_model extends CI_Model
{
    ...
    {
        $this->db->select('BaseTbl.createdAt');
    ...
    public function checkUser($data = array())
    {
        ...
        if ($check > 0) {
            ...
            if (!$this->user_model->checkEmailExists($data['email'])) {

                // Insert user data
                $insert = $this->user_model->addNewUser($data);
                ...
                $userInfo = ['updatedBy' => $query->userId, 'updatedAt' => date('Y-m-d H:i:s')];

                $userInfo['oauth_provider'] = $data['oauth_provider'];
                $userInfo['oauth_uid'] = $data['oauth_uid'];

                $result = $this->user_model->editUser($userInfo, $query->userId);
